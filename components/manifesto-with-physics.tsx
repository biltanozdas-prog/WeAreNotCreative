      ...
      >
        {SERVICES.map((svc, i) => (
          <div
            ...
            className={`absolute select-none border border-foreground overflow-hidden px-3 py-2 md:px-4 md:py-3.5 ${
          ...
          >
            <p className={`text-[7px] md:text-[8px] tracking-[.12em] mb-1 md:mb-1.5 ${svc.dark ? "text-white/30" : "text-foreground/30"}`}>
              {svc.num}
            </p>
            <p className="text-[10px] md:text-[12px] font-black tracking-[-0.015em] uppercase leading-[1.05] mb-1">
              {svc.name}
            </p>
            <p className={`text-[8px] md:text-[9px] leading-[1.25] md:leading-[1.4] line-clamp-2 ${svc.dark ? "text-white/45" : "text-foreground/45"}`}>
              {svc.desc}
            </p>
          </div>
        ))}
