...
function BlogEntry({
  ...
  return (
    <button
      onClick={onOpen}
      className={`block w-full text-left bg-transparent border-none cursor-pointer p-0 group ${
        isOffset ? "md:ml-auto md:max-w-[75%]" : "md:max-w-[85%]"
      }`}
    >
...
function ReaderPanel({
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 bg-[#000000]/50 z-[99998] transition-opacity duration-500 ${
          post ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        }`}
        onClick={onClose}
        aria-hidden="true"
      />
      ...
      <div
        className={`fixed top-0 right-0 w-full md:w-[55vw] h-screen bg-background z-[99999] transition-transform duration-500 ease-[cubic-bezier(0.85,0,0.15,1)] border-l-2 border-foreground overflow-y-auto ${
        ...
        {post && (
          <div className="px-8 py-12 md:px-12 md:py-16">
            ...
            {post.content.slice(0, 2).map((paragraph, i) => (
              <p
                key={i}
                className="font-sans font-light text-[15px] md:text-[17px] text-foreground/80 leading-[1.7] mb-6 md:mb-8"
              >
                {paragraph}
              </p>
            ))}
            ...
            {post.content.slice(2).map((paragraph, i) => (
              <p
                key={i + 2}
                className="font-sans font-light text-[15px] md:text-[17px] text-foreground/80 leading-[1.7] mb-6 md:mb-8"
              >
